...
RegisterNUICallback('getSettings', function(data, cb)
    cb({
        ...
        audio = audioEnabled,
        hideItemDetails = GetResourceKvpInt('inv_hideItemDetails') == 1,
        hideRarityBorder = GetResourceKvpInt('inv_hideRarityBorder') == 1,
    })
end)

-- persist inventory display toggles (item details / rarity border) in KVP
RegisterNUICallback('saveInventorySetting', function(data, cb)
    if data and data.key then
        SetResourceKvpInt(('inv_%s'):format(data.key), data.value and 1 or 0)
    end
    cb(1)
end)
...
RegisterNetEvent('ox_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
    ...
    client.tick = SetInterval(function()
        ...
        if invOpen then
            ...
            if vehicleDiagnosticOpen == false and currentInventory.type == 'newdrop' then
...
RegisterNetEvent('ox_inventory:viewInventory', function(left, right)
    ...
    SetInterval(client.interval, 100)
    print('keep input true from viewinventory')
    SetNuiFocus(true, true)
    SetNuiFocusKeepInput(true)
    closeTrunk()
...
RegisterNetEvent('ox_inventory:checkParachute', function(item)
    CheckParachuteItem(false)
end)

RegisterNetEvent('ox_inventory:checkArmor', function(item)
    CheckArmorItem(false)
end)
...
function CheckParachuteItem(remove)
    local restrictions = SlotRestrictions.fetch()
    local slot9 = restrictions["9"]
    local whitelistedItems = (slot9 and slot9.restrictions and slot9.restrictions.type == "allowed_items") and slot9.restrictions.items or {}
        ...
        if item then
            for _, v in pairs(item) do
                if v.slot == 9 then
                    ...
                    client.parachute = {CreateParachuteBagObject(cache.ped, true, true), v?.metadata?.type or -1}
                    if v.metadata.type then
                        SetPlayerParachuteTintIndex(PlayerData.id, v.metadata.type)
                    end
                end
            end
        end
...
function useArmor(metadata)
    ...
    if metadata.type then
        local decoded = json.decode(metadata.type)
        armor = tonumber(decoded and decoded.value)
    else
...
function CheckArmorItem(remove, to)
    if remove then
        ...
        Wait(1000)
        local restrictions = SlotRestrictions.fetch()
        local slot7 = restrictions["7"]
        local whitelistedItems = (slot7 and slot7.restrictions and slot7.restrictions.type == "allowed_items") and slot7.restrictions.items or {}

        local items = exports.ox_inventory:Search('slots', whitelistedItems)
        if items then
...
RegisterNUICallback('craftPersonalRecipe', function(data, cb)
    ...
    cb({ success = success or false, error = not success and response or nil })
end)

-- Attempt to craft a hidden recipe from items dragged into the workbench.
RegisterNUICallback('craftHiddenRecipe', function(data, cb)
...
-- Open the phone's SIM/container inventory (from the two side slots / phone slot).
...
-- Close the phone container (toggle off).
RegisterNUICallback('closePhone', function(_, cb)
    cb(1)
    client.closeInventory()
end)

-- Preview of the phone's SIM cards so the side boxes show them even when closed.
RegisterNUICallback('getPhoneContents', function(_, cb)
    cb(lib.callback.await('ox_inventory:getPhoneContents', false) or {})
end)

-- The crafting HUD is shown while the inventory (NUI) is closed, so it can't
-- receive key presses. Track when it's active and bind X to cancel the craft.
local craftingHudActive = false

RegisterNUICallback('craftingHudActive', function(data, cb)
    craftingHudActive = data and data.active or false
    cb(1)
end)

RegisterCommand('+cancelPersonalCraft', function()
    if craftingHudActive then
        SendNUIMessage({ action = 'cancelActiveCraft' })
    end
end, false)
