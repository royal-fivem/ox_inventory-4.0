...
exports('getTrunkCapacity', function(veh) 
	if veh == 0 or veh == nil then return 0 end

	local class = GetVehicleClass(veh)
	return trunks[class]
end)

exports('getMaxTrunkCapacity', function() 
	local max = 0
	for k,v in pairs(trunks) do
		if type(k) ~= 'number' then break end
		local weight = v[2]
		if weight > max then max = weight end
	end

	for k,v in pairs(trunks.models) do
		local weight = v[2]
		if weight > max then max = weight end
	end
